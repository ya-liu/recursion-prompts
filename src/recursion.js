...
var arraySum = function(array) {
  ...
  if (acc.length === 1) {
    ...
    return first + arraySum(acc.slice(1));
  }
...
var reverse = function(string) {
  ...
  if (string.length === 1) {
    ...
    firstChar = reverse(string.slice(1)).concat(firstChar);
    return firstChar;
  }
};
...
var palindrome = function(string) {
  ...
  if (string.indexOf(" ") !== -1) {
    var target = string.split(" ").join("").toLowerCase();
  } else {
    target = string.toLowerCase();
  }

  var firstChar = target[0];
  var lastChar = target[target.length - 1];

  return firstChar === lastChar && palindrome(target.slice(1, target.length - 1));
};
...
var countOccurrence = function(array, value) {
  ...
  if (currentIndex >= 0) {
    return countOccurrence(array.slice(currentIndex + 1), value) + 1;
  }
};
...
var countKeysInObj = function(obj, key) {
  ...
  for (var i = 0; i < keyArr.length; i++) {
    ...
    var v = obj[k];
    var vCount = countKeysInObj(v, key);
    counter += vCount;
